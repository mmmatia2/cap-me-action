...
Historical snapshot notice: this document captures a March 2026 contract baseline, not the current canonical contract. Reconcile against `docs/team-library-protocol.md`, `docs/STATE.md`, and current repo code before relying on it.

This snapshot predates some repo-visible contract details now present in code, including explicit schema/version constants, session sync metadata, and annotation `type` handling.
...
These retention notes were captured for the March 2026 snapshot and may not exactly match current code.
